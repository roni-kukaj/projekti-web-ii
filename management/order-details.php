...
<?php } 
    if(!isset($_GET['id'])){
        ?>
        <script>
            window.location.replace('user-info.php');
        </script>
        <?php
    }
    $total_price = 0;
    $sql = "SELECT * FROM orders_view INNER JOIN products ON orders_view.product_id = products.id WHERE orders_view.id={$_GET['id']}";
    $result = mysqli_query($conn, $sql);
    if(mysqli_num_rows($result) > 0){
    ?>
    <div class="container">
        <div class="row text-center my-4">
            <h2>Details for order number:<?php echo $_GET['id']; ?></h2>
        </div>
    <?php
